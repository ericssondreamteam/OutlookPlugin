...
namespace OutlookAddIn1
{
    class EmailFunctions
    {
        ...
        public DateTime getInflowDate()//DateTime.Today to dateTime
        {
            DateTime today = GetFirstDayOfWeek(dateTime);
        ...
        public bool isMultipleCategoriesAndAnyOfTheireInterestedUs(string categories)
        {
            ...
            else
            {
                ...
                {   
                    if (!cat.Equals("noresponsenecessary") && !cat.Equals("unknown") && !cat.Equals("") && !cat.Equals("wow") && !cat.Equals("badresponse") && !cat.Equals("goodresponse"))
                    {
                        return true;
                    }
                }
        ...
        public List<MailItem> removeDuplicateOneMoreTime(List<MailItem> emails)
        {
            ...
            for (int i = 0; i < emails.Count - 1; i++)
            {
                ...
                if (mailSubject1.StartsWith("re:") || mailSubject1.StartsWith("fw:"))
                ...
                for (int j = i + 1; j < emails.Count; j++)
                {
                    ...
                    if (mailSubject2.StartsWith("re:") || mailSubject2.StartsWith("fw:"))
            ...
            foreach (object collectionItem in oItems)
            {
                try
                {
                    ...
                    if (email1 != null)
                    {
                        ...
                        {
                            if( email1.ReceivedTime < dateTime)
                            {
